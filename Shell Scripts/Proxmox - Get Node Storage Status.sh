# Get the details of a Proxmox Node Storage and save it to a multiline and/or WYSIWYG custom field
#!/usr/bin/env bash

# Description: Get the details of a Proxmox Node Storage and save it to a multiline and/or WYSIWYG custom field
#
# Release Notes: Fixed free and total sizes not showing up correctly.
...
function formatStorage() {
    ...
    for storage in $storages; do
        ...
        storage_free_space=$(pvesh get "/nodes/localhost/storage" --noborder | grep -P "$storage_name " | awk '{print $5" "$6}')
        # Get the total space
        storage_total_space=$(pvesh get "/nodes/localhost/storage" --noborder | grep -P "$storage_name " | awk '{print $9" "$10}')
        echo -n
        echo ""
        echo "$storage"
        echo "-------------"
