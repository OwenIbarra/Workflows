# This will enable the selected administrator tools depending on your selection (Defaults to all). Can be given a comma separated list of users to exclude from this action.
...
.DESCRIPTION
    ...
    the registry editor, the run command window and task manager. You can give it a comma separated list of items if you want to enable some but not all.
    ...
    function Set-Tool {
        ...
        process {
            ...
            switch ($Tool) {
                ...
                "theControlPanel" { Set-HKProperty -Path $key\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer -Name NoControlPanel -Value 0 }
                ...
                "theRegistryEditor" { Set-HKProperty -Path $key\Software\Microsoft\Windows\CurrentVersion\Policies\System -Name DisableRegistryTools -Value 0 }
                ...
                "taskManager" { Set-HKProperty -Path $key\Software\Microsoft\Windows\CurrentVersion\Policies\System -Name DisableTaskMgr -Value 0 }
                "All" {
                    Set-HKProperty -Path $key\Software\Policies\Microsoft\Windows\System -Name DisableCMD -Value 0
                    Set-HKProperty -Path $key\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer -Name NoControlPanel -Value 0
...
process {
    ...
    if ($ExcludedUsers) {
        ...
        Write-Warning "The Following Users will not have your selected tools enabled. $ToBeExcluded"
